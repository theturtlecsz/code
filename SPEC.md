...
| 6 | SPEC-KIT-922 | Auto-commit stage artifacts | **DRAFT** | Code | docs/SPEC-KIT-922-auto-commit-stage-artifacts/spec.md | main | N/A | 2025-11-10 | Created - not started | **DRAFT** (2025-11-10, ~1 hour): Fix critical workflow gap preventing automated `/speckit.auto` completion. **Problem**: Pipeline generates stage artifacts (plan.md, tasks.md, etc.) that dirty git tree, causing guardrail failures at subsequent stages. SPEC-900 blocked at Validate stage with "clean-tree: Working tree has 1 unexpected changes". **Solution**: Auto-commit stage artifacts after each stage completes, maintaining clean tree throughout. **Implementation**: (1) New git_integration.rs module with auto_commit_stage_artifacts(), (2) Config flag spec_kit.auto_commit (default: true), (3) Integration into pipeline_coordinator.rs after consensus success, (4) Gitignore MCP side effects (.serena/, .code/). **Benefits**: 100% automated pipeline (no manual commits), guardrails work as designed (safety preserved), granular git history (6 commits per run), evidence fully preserved. **Target**: SPEC-900 re-run completes end-to-end without intervention. **Timeline**: 4 hours (implementation, testing, validation, docs). **Priority**: P0 - blocks all automated CI/CD workflows. |
