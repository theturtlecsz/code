...
| 7 | SPEC-KIT-923 | Observable agent execution via tmux | **DRAFT** | Code | docs/SPEC-KIT-923-tmux-observable-agents/spec.md | main | N/A | 2025-11-10 | Created - implementation starting | **DRAFT** (2025-11-10): Enable real-time visibility into agent execution via tmux panes. **Problem**: Agents run as black-box background processes - when they hang (like Claude 1200s timeout in SPEC-900), impossible to diagnose in real-time. No visibility into prompt processing, API calls, or failure points. **Solution**: Wrap agent CLI execution in tmux panes, one pane per agent, attachable for live monitoring. **Integration Point**: core/src/agent_tool.rs:589 execute_model_with_permissions() - single chokepoint for ALL agent CLI invocations. **Design**: Modify Command::new() to optionally use tmux wrapper, creating split panes for each agent. **Benefits**: Real-time debugging, full output capture, hung agent diagnosis, parallel execution visibility. **Timeline**: 8 hours (investigation complete, implementation next). **Priority**: P0 - critical for debugging agent timeouts and hangs. |
