...
| 11 | MAINT-11 | Refactor ChatWidget monolith | **IN PROGRESS** | Code | Maintainability | | | 2025-12-16 | `codex-rs/tui/src/chatwidget/mod.rs`, `command_render.rs`, `agent_status.rs`, `submit_helpers.rs`, `input_helpers.rs` | Break the `mod.rs` "gravity well" into smaller modules behind stable interfaces. **Phase 1 (P110)**: Extracted command_render.rs (~200 LOC, 8 tests). **Phase 2 (P113)**: Extracted agent_status.rs (~65 LOC types+helpers, 3 tests). **Phase 3 (P114)**: Extracted submit_helpers.rs (~300 LOC, 4 tests). **Phase 4 (P115)**: Dead code cleanup (8 warnings fixed). **Phase 5 (P116)**: Extracted input_helpers.rs (175 LOC, 5 tests). **Phase 6 (P117)**: Removed browser/chrome dead code (-2,094 LOC: chrome_selection_view.rs, browser handlers, screenshot rendering, unused imports). mod.rs: 23,413→20,758 LOC (-2,655 LOC, -11.3%). **Remaining**: Session handling, agent terminal, history management. |
| 12 | MAINT-12 | Stage0: NotebookLM HTTP-only + local-memory CLI/REST (no MCP) | **IN PROGRESS** | Code | SPEC-KIT-102R | | | 2025-12-15 | `codex-rs/tui/src/stage0_adapters.rs`, `codex-rs/tui/src/chatwidget/spec_kit/stage0_integration.rs`, `codex-rs/core/src/config.rs`, `codex-rs/stage0/src/config.rs`, `docs/OPERATIONAL-PLAYBOOK.md` | **Goal**: Stage0 must not require MCP at all. Use local-memory **REST health + CLI** only; use NotebookLM **HTTP** only. **Config**: notebook identifier is per-project in `~/.code/config.toml` (support `notebook`, `notebook_id`, `notebook_url`). **Compatibility**: accept notebook id + URL forms; no fallback to MCP. **Docs**: fix `/api/ask` payload keys (avoid `notebookId`). |
| 13 | MAINT-13 | Project config inheritance for subdirectories | **PENDING** | Code | SPEC-KIT-939 | | | 2025-12-15 | `codex-rs/core/src/config.rs`, `docs/config.md` | Fix mismatch where `[projects.\"/path\"]` is expected to apply to subdirectories (hooks, Stage0 config), but runtime currently uses exact cwd match only. |
| 14 | MAINT-14 | Remove repo-root `mcp.json` (unused, contains secrets) | **DONE** | Code | Config hygiene | | | 2025-12-15 | N/A | ~~Delete `mcp.json` from repo~~. **Completed**: File already removed (verified 2025-12-15). MCP servers now configured via `~/.code/config.toml` `[mcp_servers]`. |
| 15 | MAINT-15 | P111 config fixes + P112 regression fix | **DONE** | Code | Config hygiene | | | 2025-12-15 | `config_types.rs:395-438`, `claude_streaming.rs:20-60`, `gemini_streaming.rs:21-68`, `mcp_consensus_benchmark.rs` | **P111 (424990cc3)**: (1) MCP timeout schema alignment (`startup_timeout_sec`, `tool_timeout_sec` with `_sec` precedence over legacy `_ms`), (2) Per-model CLI provider caches (`Arc<HashMap<String, Arc<Provider>>>` pattern), (3) Docs fixes (history path, MAINT-14 closure), (4) Dead code removal (`install-templates.sh`). **P112**: Fixed regression in `mcp_consensus_benchmark.rs:30` (missing new struct fields). **Verification**: 2230 tests pass, build succeeds. **P113**: Stage0 clippy warnings fixed (auto-fix + test-utils feature). |
...
| 5 | SPEC-KIT-921 | CLI Adapter + SpeckitExecutor Core | **DONE** | Code | docs/spec-kit/CLI-REFERENCE.md | main | 0ff020290 | 2025-12-22 | 1771 LOC, all 11 subcommands | **COMPLETE** (2025-12-22): CLI adapter with shared SpeckitExecutor core enabling headless spec-kit operations. **Delivered**: (1) SpeckitExecutor in spec-kit/src/executor/ (Outcome types, shared model), (2) CLI wiring in cli/src/speckit_cmd.rs (1771 LOC), (3) All 11 subcommands (status, review, specify, plan, tasks, implement, validate, audit, unlock, run, migrate), (4) CI integration (.github/workflows/spec-kit-ci.yml tests), (5) CLI-REFERENCE.md documentation (300+ lines). **Supersedes**: SPEC-KIT-920 (PTY automation - wrong approach). **Commits**: 0ff020290, aa8b6439d. **Impact**: Enables headless orchestration, CI validation, scripted pipelines. |
...
| 5 | SYNC-005 | Add keyring-store crate | **Done** | Code | docs/UPSTREAM-ANALYSIS-2025-11-27.md | main | | 2025-12-14 | | 241 LOC. KeyringStore + MockKeyringStore. **Integrated** in login crate (DeviceCodeTokenStorage). |
| 6 | SYNC-006 | Add feedback crate | **Done** | Code | docs/UPSTREAM-ANALYSIS-2025-11-27.md | main | | 2025-12-14 | | 306 LOC, 6 tests. Ring buffer + tracing. **Integrated** in TUI (get_feedback_collector, /feedback command). |
...
- **Fork Management**: FORK_DEVIATIONS.md, docs/TUI.md
