...
**STATUS**: 0/7 Started (7 PRDs ready for execution)

| Order | Task ID | Title | Status | Owners | PRD | Branch | PR | Created | Notes |
|-------|---------|-------|--------|--------|-----|--------|----|---------|-------|
| 1 | SPEC-KIT-933 | Database Integrity & Hygiene | **BACKLOG** | Code | docs/SPEC-KIT-933-database-integrity-hygiene/PRD.md | | | 2025-11-13 | **P0 - CRITICAL** (Data Corruption Risk): ACID transactions eliminate dual-write corruption, incremental auto-vacuum reduces 153MB→<5MB (96%), parallel spawning 150ms→50ms (3×), daily cleanup automation. **Estimated Effort**: 65-96 hours (2-3 weeks). **Blocks**: SPEC-934 (transactions enable safe migration). |
| 2 | SPEC-KIT-934 | Storage Consolidation | **BACKLOG** | Code | docs/SPEC-KIT-934-storage-consolidation/PRD.md | | | 2025-11-13 | **P1 - HIGH** (Policy Violation + Performance): Migrate consensus artifacts MCP→SQLite (5× faster: 30ms vs 150ms), eliminate MCP from orchestration, restore SPEC-KIT-072 compliance, reduce 4 storage systems→2. **Estimated Effort**: 10-13 hours (2-3 days). **Dependencies**: SPEC-933 (transactions enable safe migration). **Blocks**: SPEC-936 (filesystem elimination depends on this). |
| 3 | SPEC-KIT-936 | Tmux Elimination & Async Orchestration | **BACKLOG** | Code | docs/SPEC-KIT-936-tmux-elimination/PRD.md | | | 2025-11-13 | **P1 - HIGH** (Performance + Technical Debt): Direct async API calls eliminate tmux overhead (target 65× speedup: 6.5s→0.1s), remove filesystem collection, investigate OAuth2 device code flows. **Measurement Gap Acknowledged**: Claims ESTIMATED not MEASURED (proceed with risk acceptance, SPEC-940 validates). **Estimated Effort**: 45-65 hours (2-3 weeks). **Enables**: SPEC-934 filesystem cleanup. |
| 4 | SPEC-KIT-938 | Enhanced Agent Retry Logic | **BACKLOG** | Code | docs/SPEC-KIT-938-enhanced-agent-retry/PRD.md | | | 2025-11-13 | **P2 - MEDIUM** (Reliability + UX): Error classification (retryable vs permanent), exponential backoff with jitter, quality gate integration (3/3 consensus via retry), comprehensive telemetry. Beyond AR-2/3/4 implementation. **Estimated Effort**: 4-6 hours (1 day). |
| 5 | SPEC-KIT-939 | Configuration Management | **BACKLOG** | Code | docs/SPEC-KIT-939-configuration-management/PRD.md | | | 2025-11-13 | **P2 - MEDIUM** (UX + Flexibility): Hot-reload config when idle, canonical name field, startup validation, configurable quality gate agents, pluggable validation layers, JSON Schema documentation, API key naming guide. **Estimated Effort**: 22-32 hours (1-1.5 weeks). |
| 6 | SPEC-KIT-940 | Performance Instrumentation | **BACKLOG** | Code | docs/SPEC-KIT-940-performance-instrumentation/PRD.md | | | 2025-11-13 | **P2 - MEDIUM** (Validation + Optimization Foundation): Timing infrastructure (measure_time! macro), benchmark harness (n≥10 runs, statistics), pre/post validation for SPEC-933/934/936, statistical reporting (mean±stddev, p<0.01). **Validates**: All performance claims MEASURED not ESTIMATED. **Estimated Effort**: 12-16 hours (2-3 days). **Dependencies**: SPEC-936 (validates claims post-implementation). |
| 7 | SPEC-KIT-941 | Automated Policy Compliance | **BACKLOG** | Code | docs/SPEC-KIT-941-automated-policy-compliance/PRD.md | | | 2025-11-13 | **P2 - MEDIUM** (Quality + Policy Enforcement): CI checks for SPEC-KIT-072 violations, static analysis for storage separation, pre-commit hooks (optional), policy compliance dashboard, tag schema validation. **Prevents**: Regression after SPEC-934 fixes policy violation. **Estimated Effort**: 8-10 hours (1-2 days). **Dependencies**: SPEC-934 (fixes initial violation, this prevents recurrence). |

**Total Effort**: 167-239 hours (4-6 weeks at 40h/week)
...
| 5 | SPEC-KIT-921 | Tmux orchestration testing and validation | **DRAFT** | Code | docs/SPEC-KIT-921-tmux-orchestration-testing/spec.md | main | N/A | 2025-11-10 | Created - not started | **DRAFT** (2025-11-10, ~1 hour): Fix critical tmux automation bug blocking SPEC-900 automated validation. **Problem**: Manual TUI `/speckit.auto SPEC-KIT-900` works ✅, tmux-automated run exits immediately after 6-8s ❌. **Root Cause**: Command delivery or timing issue in tmux-automation.sh (not code logic). **Hypotheses**: (1) Insufficient init delay before send-keys, (2) Command delivery mechanism (Enter vs C-m), (3) Timing-sensitive state checks, (4) Environment differences. **Approach**: (1) Enhanced logging/diagnostics, (2) Hypothesis testing (timing, delivery, state, env), (3) Fix implementation, (4) SPEC-900 validation. **Target**: 100% success rate for tmux-automated long-running commands (60+ min). **Timeline**: 8 hours (4 phases). **Priority**: P1 - blocks automated CI/CD testing for full pipelines. |
...
| 10 | SPEC-KIT-926 | TUI progress tracking and status visibility | **DRAFT** | Code | docs/SPEC-KIT-926-tui-progress-visibility/spec.md | main | N/A | 2025-11-11 | Created - planning | **DRAFT** (2025-11-11): Comprehensive TUI visibility improvements to eliminate "what is it doing?" confusion. **Problem**: Users have zero visibility during operations - no upfront summary, silent execution periods, unclear waiting vs hung states, hidden agent work. **Solution**: (1) Pipeline preview before execution, (2) Status bar showing current operation, (3) Agent execution dashboard with heartbeat indicators, (4) Sequential progress tracker, (5) Multi-stage pipeline progress bars, (6) Consensus synthesis visibility, (7) Rich error context. **Scope**: 7 phases, ~15-18 hours implementation. **User Stories**: 6 detailed scenarios showing expected UX. **Implementation Plan**: StatusBar widget, agent dashboard, progress tracker, preview confirmation, enhanced error display. **Success Criteria**: No silent periods >15s, clear current operation always visible, progress awareness at any point. **Priority**: High - critical UX issue affecting all /speckit.* commands. |
