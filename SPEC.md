...
| 11 | MAINT-11 | Refactor ChatWidget monolith | **IN PROGRESS** | Code | Maintainability | | | 2025-12-15 | `codex-rs/tui/src/chatwidget/mod.rs`, `command_render.rs`, `agent_status.rs`, `submit_helpers.rs` | Break the `mod.rs` "gravity well" into smaller modules behind stable interfaces. **Phase 1 (P110)**: Extracted command_render.rs (~200 LOC, 8 tests). **Phase 2 (P113)**: Extracted agent_status.rs (~65 LOC types+helpers, 3 tests). **Phase 3 (P114)**: Extracted submit_helpers.rs (~300 LOC, 4 tests). **Phase 4 (P115)**: Dead code cleanup (8 warnings fixed: removed unused show_prd_builder/PrdBuilderModal::new/build_stage0_context_prefix, annotated planned features). mod.rs: 23,413→22,906 LOC (~507 LOC extracted/removed). **Note**: Slash-command routing already well-modularized (slash_command.rs + app.rs dispatch). **Remaining**: Input helpers, event handlers. |
