...
|-------|--------|--------|----|----------|-------|
| `/speckit.tasks` | **Done** | main | - | 2025-10-28 | 9-task matrix captured in `docs/SPEC-KIT-900-generic-smoke/{spec,tasks}.md`; telemetry/cost schema, security template, consensus playbook, QA sweep, adoption dashboard delivered. Branch merged and cleaned up 2025-11-29. |
...
| 6 | SPEC-KIT-940 | Performance Instrumentation | **IN PROGRESS** | Code | docs/SPEC-KIT-940-performance-instrumentation/PRD.md | | | 2025-11-28 | **P2 - MEDIUM** ✅ Phase 1/4 COMPLETE + Quick Validation. **Complete**: (1) timing.rs module (49 LOC core, 296 LOC spec-kit with Timer struct + 10 tests), (2) Quick validation test added to async_agent_executor.rs. **Validation Result** (2025-11-28): DirectProcessExecutor 0.1ms mean spawn overhead (n=10, stddev 0.3ms) - **500x better than <50ms target**. SPEC-936 claim validated. **Remaining**: Phase 2 (BenchmarkHarness struct, 3-4h), Phase 3 (SQLite/MCP baselines, 3-4h), Phase 4 (Statistical reporting + CI, 3-5h). **Progress**: 9-13h remaining. |
