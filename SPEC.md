...
| 11 | MAINT-11 | Refactor ChatWidget monolith | **IN PROGRESS** | Code | Maintainability | | | 2025-12-23 | `codex-rs/tui/src/chatwidget/mod.rs`, `command_render.rs`, `agent_status.rs`, `submit_helpers.rs`, `input_helpers.rs`, `review_handlers.rs`, `session_handlers.rs`, `undo_snapshots.rs`, `pro_overlay.rs` | Break the `mod.rs` "gravity well" into smaller modules behind stable interfaces. **Phase 1-5**: Extracted command_render, agent_status, submit_helpers, input_helpers + dead code cleanup. **Phase 6**: Removed browser/chrome dead code (-2,094 LOC). **Phase 7**: Extracted review_handlers.rs (462 LOC). **Phase 8**: Extracted session_handlers.rs (619 LOC). **Phase 9**: Extracted undo_snapshots.rs (497 LOC). **Phase 10**: Wired pro_overlay.rs (619 LOC), removed ~570 LOC duplicates. mod.rs: 23,413→18,040 LOC (-5,373 LOC, -22.9%). **Remaining**: Agent terminal, history management (lower priority). |
| 12 | MAINT-12 | Stage0: NotebookLM HTTP-only + local-memory CLI/REST (no MCP) | **DONE** | Code | SPEC-KIT-102R | main | db3985846 | 2025-12-23 | `codex-rs/tui/src/stage0_adapters.rs` | **COMPLETE**: Stage0 uses HTTP-only for NotebookLM (`Tier2HttpAdapter`) and CLI/REST for local-memory (`LocalMemoryCliAdapter`). Removed dead `Tier2McpAdapter` code (~70 LOC), `parse_tier2_response`, `NOTEBOOKLM_SERVER` constant, `McpConnectionManager` import. Config supports `notebook`, `notebook_id`, `notebook_url` via serde aliases. CONVERGENCE alignment verified. |
| 13 | MAINT-13 | Project config inheritance for subdirectories | **DONE** | Code | SPEC-KIT-939 | | | 2025-12-23 | `codex-rs/core/src/config.rs` | **ALREADY IMPLEMENTED**: `resolve_project_override()` uses `starts_with()` for ancestor matching + deepest-match selection. Added 2 verification tests: `project_stage0_config_inherits_to_subdirectories`, `project_hooks_inherit_to_subdirectories`. All 3 inheritance tests pass (trust, stage0, hooks). No code changes needed—issue was outdated. |
