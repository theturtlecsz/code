...
| 11 | MAINT-11 | Refactor ChatWidget monolith | **IN PROGRESS** | Code | Maintainability | | | 2025-12-15 | `codex-rs/tui/src/chatwidget/mod.rs`, `command_render.rs`, `agent_status.rs` | Break the `mod.rs` "gravity well" into smaller modules behind stable interfaces. **Phase 1 (P110)**: Extracted command_render.rs (~200 LOC, 8 tests). **Phase 2 (P113)**: Extracted agent_status.rs (~65 LOC types+helpers, 3 tests). mod.rs: 23,413→23,151 LOC. **Remaining**: Input submission, slash-command routing. |
