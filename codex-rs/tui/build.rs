fn main() {
    ...
    fn humanize(name: &str) -> String {
        ...
        out.split_whitespace()
            .map(|w| {
                let mut cs = w.chars();
                match cs.next() {
                    Some(f) => format!("{}{}", f.to_uppercase(), cs.as_str().to_lowercase()),
                    None => String::new(),
                }
            })
            .collect::<Vec<_>>()
    ...
    fn group_for(name: &str) -> String {
        ...
        let key = n.as_str();
        let g = if key.contains("dot") {
            "Dots"
        } else if key.contains("circle") || key.contains("round") || key.contains("arc") {
            "Circles"
        ...
        {
            "Lines"
        } else if key.contains("bounce") || key.contains("ball") || key.contains("pong") {
            "Bouncing"
        } else if key.contains("star") || key.contains("asterisk") {
            "Stars"
        } else if key.contains("arrow") || key.contains("triangle") {
            "Arrows"
        } else if key.contains("box") || key.contains("square") {
            "Boxes"
        } else if key.contains("toggle") {
            "Toggles"
        } else if key.contains("monkey")
            || key.contains("earth")
            || key.contains("moon")
