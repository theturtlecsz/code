...
pub fn default_for(name: &str) -> Option<SubagentCommandConfig> {
    match name {
        ...
        "speckit.plan" => Some(make_config(
            name,
            true,
            &["gemini-flash", "claude-haiku", "gpt5_1"],
            "Run the plan stage using tier-2 routing (gemini-flash, claude-haiku, gpt-5.1 multi-agent consensus). Provide the SPEC packet and clarify/checklist outputs as context, then ensure each agent emits the spec-plan JSON and records it via local-memory with tags `spec:<spec_id>` and `stage:plan`. Capture consensus agreements/conflicts before replying.",
        ...
        "speckit.implement" => Some(make_config(
            name,
            false,
            &["gpt5_1_codex", "claude-haiku"],
            "Execute the implementation stage with tier-2 routing: gpt-5.1-codex (code generation specialist) + claude-haiku (validation). Allow write access in isolated worktrees. Supply prior plan/tasks context and require each agent to push its JSON artifact to local-memory tagged `spec:<spec_id>` and `stage:implement`.",
        ...
        "speckit.validate" => Some(make_config(
            ...
            true,
            &["gemini-flash", "claude-haiku", "gpt5_1"],
            "Run validation with tier-2 routing (gemini-flash, claude-haiku, gpt-5.1 multi-agent consensus). Provide telemetry logs, test outputs, and plan/tasks context. Ensure each agent stores its validation JSON to local-memory tagged `spec:<spec_id>` and `stage:validate` with importance >= 8.",
        ...
        "speckit.audit" => Some(make_config(
            name,
            true,
            &["gpt5_codex", "claude-sonnet", "gemini-pro"],
            "Launch tier-3 routing (gpt-5-codex, claude-sonnet, gemini-pro premium agents) to complete the audit stage. Scope context to diffs, telemetry, and evidence directory footprint. Require local-memory persistence tagged `spec:<spec_id>` and `stage:audit` for each agent artifact before synthesising the decision.",
        ...
        "speckit.unlock" => Some(make_config(
            ...
            true,
            &["gpt5_codex", "claude-sonnet", "gemini-pro"],
            "Use tier-3 routing (gpt-5-codex, claude-sonnet, gemini-pro premium agents) for unlock stage. Provide branch state, outstanding work, and guardrail results for the spec_id. Persist each unlock JSON artifact to local-memory tagged `spec:<spec_id>` and `stage:unlock` before summarising the final decision.",
