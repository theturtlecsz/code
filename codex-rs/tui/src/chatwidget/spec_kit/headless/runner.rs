...
impl HeadlessPipelineRunner {
    ...
    pub fn new_with_backend(
        ...
        let run_id = crate::chatwidget::spec_kit::execution_logger::generate_run_id(&spec_id);
        Self {
            spec_id,
            from_stage,
            to_stage,
    ...
    pub fn run(&mut self) -> HeadlessResult {
        ...
        for stage in stages {
            ...
            // Step 2b: Quality gate (LOCK E2 - check BEFORE stage execution)
            // Gates are evaluated at the boundary before a stage starts
            ...
            // Step 2c: Execute stage (spawn agents, wait for completion)
    ...
    fn write_stage_output(
        ...
        let spec_stage = match stage {
            Stage::Plan => SpecStage::Plan,
            Stage::Tasks => SpecStage::Tasks,
            Stage::Implement => SpecStage::Implement,
            Stage::Validate => SpecStage::Validate,
            Stage::Audit => SpecStage::Audit,
            Stage::Unlock => SpecStage::Unlock,
            _ => return Ok(()),
        };

        super::super::stage_synthesis::synthesize_from_cached_responses(
            outputs,
            &self.spec_id,
            spec_stage,
            &self.cwd,
            Some(&self.run_id),
            super::super::stage_synthesis::SynthesisSideEffects {
    ...
    /// Run quality gate for a stage (MAINT-930-B)
    ///
    /// Maps stage to checkpoint and checks if enabled in config.
    /// Returns NeedsApproval in headless mode when checkpoint is enabled.
    ///
    /// LOCK E2: Canonical checkpoint boundary mapping:
    /// - quality_gates.plan (PrePlan) ⇒ before executing Stage::Plan
    /// - quality_gates.tasks (PostPlan) ⇒ after Stage::Plan / before Stage::Tasks
    /// - quality_gates.validate (PostTasks) ⇒ after Stage::Tasks / before Stage::Implement
    /// - quality_gates.audit (PostValidate) ⇒ after Stage::Validate / before Stage::Audit
    /// - quality_gates.unlock (PreUnlock) ⇒ after Stage::Audit / before Stage::Unlock
    fn run_quality_gate(&self, stage: &Stage) -> Result<(), HeadlessError> {
        // Map stage to checkpoint name per LOCK E2
        // Gates are evaluated BEFORE the stage starts, at the defined boundary
        let checkpoint = match stage {
            Stage::Specify => {
                // No quality gate before Specify
                tracing::debug!(stage = %stage.as_str(), "No quality gate for Specify");
                return Ok(());
            }
            Stage::Plan => "plan",          // PrePlan: before Plan
            Stage::Tasks => "tasks",        // PostPlan: before Tasks
            Stage::Implement => "validate", // PostTasks: before Implement
            Stage::Validate => {
                // No checkpoint between Implement and Validate per LOCK E2
                tracing::debug!(stage = %stage.as_str(), "No quality gate for Validate");
                return Ok(());
            }
            Stage::Audit => "audit",   // PostValidate: before Audit
            Stage::Unlock => "unlock", // PreUnlock: before Unlock
        };

        // Check if checkpoint is enabled in config
        let is_enabled = self.is_checkpoint_enabled(checkpoint);

        if !is_enabled {
            tracing::debug!(
                stage = %stage.as_str(),
                checkpoint = %checkpoint,
                "Quality gate checkpoint disabled"
            );
            return Ok(());
        }

        // In headless mode, enabled checkpoints require approval
        tracing::info!(
            stage = %stage.as_str(),
            checkpoint = %checkpoint,
            "Quality gate requires approval in headless mode"
        );

        Err(HeadlessError::NeedsApproval {
            checkpoint: checkpoint.to_string(),
        })
    }

    /// Check if a quality gate checkpoint is enabled (MAINT-930-B)
    fn is_checkpoint_enabled(&self, checkpoint: &str) -> bool {
        let Some(ref quality_gates) = self.planner_config.quality_gates else {
            return false; // No quality gates config = disabled
        };

        let checkpoint_config = match checkpoint {
            "plan" => &quality_gates.plan,
            "tasks" => &quality_gates.tasks,
            "validate" => &quality_gates.validate,
            "audit" => &quality_gates.audit,
            "unlock" => &quality_gates.unlock,
            _ => return false,
        };

        // Strict semantics: `enabled=true` is enabled, regardless of agents list.
    ...
    ///
    /// Uses shared prompt_guard module for atomic counting (MAINT-930-A).
    #[allow(dead_code)]
    fn guard_prompt<T>(&self) -> Result<T, HeadlessError> {
        crate::chatwidget::spec_kit::prompt_guard::increment_prompt_attempts();
