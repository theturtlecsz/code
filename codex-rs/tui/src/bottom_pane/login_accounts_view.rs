...
impl LoginAccountsState {
    ...
    fn reload_accounts(&mut self) {
        match auth_accounts::list_accounts(&self.codex_home) {
            Ok(raw_accounts) => {
                let active_id = auth_accounts::get_active_account_id(&self.codex_home)
                    .ok()
                    .flatten();
    ...
    fn sync_account_store_from_auth(&mut self) {
        let auth_file = auth::get_auth_file(&self.codex_home);
    ...
    fn activate_account(&mut self, account_id: String, mode: AuthMode) -> bool {
        match auth::activate_account(&self.codex_home, &account_id) {
            Ok(()) => {
                self.feedback = Some(Feedback {
                    message: match mode {
    ...
    fn remove_account(&mut self, account_id: String) {
        match auth_accounts::remove_account(&self.codex_home, &account_id) {
            Ok(Some(_)) => {
                ...
                if removed_active {
                    let _ = auth::logout(&self.codex_home);
                }
                self.feedback = Some(Feedback {
                ...
                let using_chatgpt = self
                    ...
                    .as_ref()
                    .and_then(|id| {
                        auth_accounts::find_account(&self.codex_home, id)
                            .ok()
                            .flatten()
                    })
            ...
            AddStep::ApiKey { field } => match key_event.code {
                ...
                KeyCode::Enter => {
                    ...
                    if key.is_empty() {
                        ...
                        match auth::login_with_api_key(&self.codex_home, &key) {
                            Ok(()) => {
                                self.feedback = Some(Feedback {
