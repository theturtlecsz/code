...
## SPEC-KIT-920 Status (Automation) — COMPLETED

**Status:** Completed 2026-01-19

**Delivered:**

* `--initial-command` dispatches slash command after first redraw
* `--exit-on-complete` exits after command completes via AutomationSuccess/AutomationFailure events
* Exit codes: 0 for success, non-zero for failure (D113/D133 parity)

**Implementation:** Uses `AutomationSuccess`/`AutomationFailure` events in `app_event.rs`, `app.rs`, and pipeline coordinator.
