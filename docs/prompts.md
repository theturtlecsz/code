...
- How to use:
  - Start a new session (Planner loads custom prompts on session start).
...
- Notes:
  ...
  - New or changed files are discovered on session start. If you add a new prompt while Planner is running, start a new session to pick it up.
