**SPEC-ID**: SYNC-024
**Feature**: TUI /ps command (background terminal/process visibility)
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-024
**Owner**: Code

**Context**: Add a /ps command to list background exec/PTY sessions with terminal-aware navigation and basic management actions.
...
**Story**: As a user, I want tui /ps command (background terminal/process visibility) so that the system becomes more capable without regressions.
...
- **FR1**: Implement TUI /ps command (background terminal/process visibility) behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-024/

---

## Dependencies

- SYNC-023 (preferred: shared rendering patterns)
- Exec registry (existing)

---

## Notes

Keep scope: list + focus; kill actions only if already safe/approved in your exec policy.
