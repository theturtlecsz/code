**SPEC-ID**: SYNC-020
**Feature**: Skills v1 (discover + browse + reference injection)
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-020
**Owner**: Code

**Context**: Backport upstream Skills discovery (SKILL.md) and basic TUI browsing so users can see available skills and reference them without expanding the prompt by default.
...
**Story**: As a user, I want skills v1 (discover + browse + reference injection) so that the system becomes more capable without regressions.
...
- **FR1**: Implement Skills v1 (discover + browse + reference injection) behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-020/
...
Phase 1: metadata only (name/description/path). Do NOT inject full skill bodies automatically. Add /skills UX.
