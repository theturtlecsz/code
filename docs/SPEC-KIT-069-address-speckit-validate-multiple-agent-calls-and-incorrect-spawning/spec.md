...
| **T69-04** | Gate `on_spec_auto_agents_complete()` and `check_consensus_and_advance_spec_auto()` on active run-id to ignore late callbacks (`codex-rs/tui/src/chatwidget/spec_kit/handler.rs:599`, `handler.rs:761`). | T69-01 – T69-03 | Integration: `spec_auto_e2e::validate_manual_auto_coexistence`; verifies single consensus transition. | Document callback guard rationale in `docs/spec-kit/speckit.auto-automation.md`. |
...
| **T69-08** | Emit operator-facing dedupe notice when validate is already running (`codex-rs/tui/src/chatwidget/spec_kit/handler.rs:518`). | T69-03 | Manual check: re-trigger `/speckit.validate` during active run; ensure no agent dispatch. | Add UX string to `docs/spec-kit/speckit.auto-automation.md`. |
