**SPEC-ID**: SYNC-021
**Feature**: Skills v2 (SkillsManager + skills/list op + caching)
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-021
**Owner**: Code

**Context**: Introduce SkillsManager and a list operation so both TUI1 and TUI2 can query skills consistently, with caching and (optional) reload.
...
**Story**: As a user, I want skills v2 (skillsmanager + skills/list op + caching) so that the system becomes more capable without regressions.
...
- **FR1**: Implement Skills v2 (SkillsManager + skills/list op + caching) behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-021/
...
This is about stable interfaces: manager owns discovery + validation; UI asks via a list op.
