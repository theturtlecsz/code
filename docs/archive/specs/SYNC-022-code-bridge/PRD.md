**SPEC-ID**: SYNC-022
**Feature**: Code-Bridge integration (subscribe + surface events in sessions)
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-022
**Owner**: Code

**Context**: Consume code-bridge protocol v2 configuration (.code/code-bridge.json) and surface bridge events in the session timeline without automatically injecting into the model context.
...
**Story**: As a user, I want code-bridge integration (subscribe + surface events in sessions) so that the system becomes more capable without regressions.
...
- **FR1**: Implement Code-Bridge integration (subscribe + surface events in sessions) behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-022/

---

## Dependencies

- SYNC-019 (feature gating)

---

## Notes

Treat bridge events as a diagnostics lane by default. Add batching/backpressure to avoid flooding TUI.
