**SPEC-ID**: SYNC-030
**Feature**: Governance: requirements.toml + ExternalSandbox policy
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-030
**Owner**: Code

**Context**: Port upstream governance controls to constrain what tools can run and under what sandbox/policy conditions.
...
**Story**: As a user, I want governance: requirements.toml + externalsandbox policy so that the system becomes more capable without regressions.
...
- **FR1**: Implement Governance: requirements.toml + ExternalSandbox policy behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-030/
...
Keep it opt-in and explicit. Provide clear user errors for policy violations.
