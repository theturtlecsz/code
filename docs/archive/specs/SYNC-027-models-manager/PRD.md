**SPEC-ID**: SYNC-027
**Feature**: ModelsManager + Remote Models refresh/caching
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-027
**Owner**: Code

**Context**: Introduce upstream ModelsManager for dynamic model catalogs, caching, and (optional) remote refresh behind feature flags.
...
**Story**: As a user, I want modelsmanager + remote models refresh/caching so that the system becomes more capable without regressions.
...
- **FR1**: Implement ModelsManager + Remote Models refresh/caching behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-027/
...
Ship in a safe default mode: use current local model list unless remote_models is enabled.
