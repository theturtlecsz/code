**SPEC-ID**: SYNC-028
**Feature**: TUI v2 scaffold (tui2 build + CLI gating)
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-028
**Owner**: Code

**Context**: Bring in upstream tui2 (viewport-based TUI) as an optional frontend behind features.tui2 without breaking the existing TUI.
...
**Story**: As a user, I want tui v2 scaffold (tui2 build + cli gating) so that the system becomes more capable without regressions.
...
- **FR1**: Implement TUI v2 scaffold (tui2 build + CLI gating) behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-028/
...
Do not aim for full parity here. Goal is: builds, launches, exits cleanly; stable TUI remains default.
