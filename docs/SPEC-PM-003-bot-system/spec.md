...
* Automatic PM state transitions **based on bot recommendations** (bots recommend; PM acts), except the locked holding-state **auto-return** behavior (D140).
...
* **Bot job management service runtime** (D135; clarified by D142): lightweight persistent service is allowed for bot job management; must be systemd-managed and auto-resume on reboot; must avoid heavyweight frameworks and always-processing loops. The service may remain idle and running while the user is logged in (D142).
...
**Confirmed (2026-02-12 Q\&A session)**

* **Service startup** (D141/PM-D33): systemd unit uses `WantedBy=default.target`. Service starts on login, checks for stale/incomplete runs immediately.
* **Idle behavior** (D142/PM-D34): service never exits while logged in. D135 reinterpreted as "no always-processing loops", not "exit when idle."
* **Cache trust model** (D143/PM-D35): TUI reads local cache for fast status queries. All writes go through capsule (SoR). Cache is rebuilt from capsule state if stale, missing, or corrupted.

**Idle posture (resolved)**

* The 2026-02-09 transcript proposed "never exit." The 2026-02-12 session confirmed this as PM-D34/D142: the constraint in D135 is against always-*processing* loops and heavy frameworks, not against a lightweight idle process. The service stays running until logout/shutdown.

## Service-First Runtime (D135; clarified by D142)

Bot runs may be **long-lived** (hours → days) and must survive TUI restarts, process crashes, and machine reboots.

Baseline runtime (D135; ADR-004; D142): a lightweight **PM bot service** managed by a **systemd user unit** that can resume incomplete runs without user interaction and stays running while the user is logged in (idle is acceptable; no always-processing loops). The ephemeral CLI runner remains as fallback/debug, not the primary execution model.
...
* **Service-first runtime (systemd-managed)** (baseline; D135, ADR-004; clarified by D142):
  * Pros: supports long-lived runs + reboot survival; isolates runs from TUI lifetime; enables resume/cancel semantics.
  * Cons: introduces IPC + lifecycle management; must strictly enforce D135 scope (no always-processing loops; no heavy frameworks; job-scoped runtime).
...
Baseline runtime: **PM BotRunnerService** + **systemd user unit** (D135; ADR-004; D142).
...
* The service must remain “lightweight” and job-scoped: no heavy agent framework, and no always-processing loops; idle while logged in is acceptable (D142).
...
### Local Cache Trust Model (D143/PM-D35)

* **Reads**: TUI and CLI read from the local cache (`pm://` namespace) for fast status queries. Cache contains projected run state, checkpoint summaries, and work item metadata.
* **Writes**: all mutations (run submission, state transitions, checkpoint persistence) go through capsule first, then update the local cache (dual-write).
* **Staleness**: if local cache is missing, corrupted, or older than the capsule head, rebuild from capsule. No staleness indicator in UX for v1 — cache is assumed fresh if the service is running (the service is the only writer).
* **Service down**: TUI degrades to reading capsule directly (slower but always consistent).
...
* ~~Should BLOCKED become a dedicated headless exit code~~ Resolved: exit 2 + structured `blocked_reason` (D139).
* Which commands are on the initial allowlist for `NeedsReview`, and how do we represent policy overrides without violating D119/D125? **Deferred** (no real engines in next cycle; PM-D30).
* ~~Resume trigger~~ Resolved: `WantedBy=default.target` (D141/PM-D33).
* ~~Idle behavior~~ Resolved: never exit (D142/PM-D34).
* What is the exact artifact schema for "finalization rebase conflict" details and manual resolution instructions? **Deferred** (no real engines in next cycle; PM-D30).
* Checkpoint frequency tuning (hybrid event + 30min floor proposed; not blocking).

## Implementation Sequencing (updated 2026-02-12)

* **Phase 0 (complete)**: walking skeleton — service + IPC + stub engines + tests (PR #60).
* **Phase 1 (in progress)**: capsule dual-write + local cache + persistence hardening + `--wait` push notifications.
* **Phase 1.5 (next)**: full work item hierarchy (all 4 levels; D137) + run configuration surface (presets + scopes; PM-D31) + TUI PM UX (PM-004) + checkpoint visibility (PM-D32).
* **Phase 2 (deferred)**: wire research/review engines + evidence-pack snapshotting + permission enforcement + worktree staging + rebase boundaries.
* **Phase 3 (deferred)**: game-day drills (offline, auth down, lock contention, partial failures, reboot resume).

## Risks (initial)

* Service/runtime drift: the bot service must remain strictly within D135 scope (systemd-managed; no always-processing loops; no heavyweight frameworks); D142 allows idle-running while logged in.
