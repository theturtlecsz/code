...
* Run branch lifecycle (default, per D73/D74):
  ...
  * On successful Unlock, auto‑merge the run branch into `main` using curated semantics and record a `BranchMerged` event referencing the run\_id.
...
* As‑of retrieval: query capsule "view" at a checkpoint (and optionally branch\_id).
...
* `/speckit.timeline` shows ordered checkpoints and stage transitions for a run, including: stage, timestamp, run\_id/spec\_id, policy snapshot id, active branch, and (if available) model versions.
* As‑of queries:
...
* Branching semantics:
  ...
  * After merge, main search DOES return the promoted artifacts, and the merge is recorded as an event with run\_id.
