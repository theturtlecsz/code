**SPEC-ID**: SYNC-031
**Feature**: Observability (minimal): tracing spans + optional OTel export
**Status**: Backlog
**Created**: 2025-12-22
**Branch**: feature/sync-031
**Owner**: Code

**Context**: Optional minimal observability wiring to support incident debugging without recreating the previously removed heavy metrics system.
...
**Story**: As a user, I want observability (minimal): tracing spans + optional otel export so that the system becomes more capable without regressions.
...
- **FR1**: Implement Observability (minimal): tracing spans + optional OTel export behind an explicit feature flag or config.
...
**Evidence Path**:
- docs/SPEC-OPS-004-integrated-coder-hooks/evidence/commands/SYNC-031/
...
Strictly minimal: spans around model calls/tool exec/session IO; exporter optional; no always-on server.
